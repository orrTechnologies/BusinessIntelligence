...
namespace BusinessInsights.Extensions
{
    ...
    public static class DynamicExtension
    {
        ...
        static object FindMatchingChildPropertiesRecursively(KeyValuePair<string, object> entry, propertycontainer DestinationPropertyInfo)
        {
            object mappedValue = null;
            //drill down on level to obtain a list of properties from the child set of properties 
            //of the dynamic Object
            var childproperties = entry.Value as IDictionary<string, object>;
            //if (entry.Key == DestinationPropertyInfo.facebookparent)
            //{
                //This is a matching parent JsonObject so now lets see if we have a matching child property in this Json Object entry
                mappedValue = (from KeyValuePair<string, object> item in childproperties
                               where item.Key == DestinationPropertyInfo.facebookfield
                               select item.Value).FirstOrDefault();
            //}
